...
class phyModel(object):
	def __init__(self,argv):
		...
		self.FRAME_LENGTH = argv['PACKET_LENGTH'] - self.PREAMBLE_LENGTH
		self.DISMIN = 1
		self.DISMAX = 20
		self.NF = self.calNoiseFloor()
		self.S = [[6.0,-3.3],[-3.3,3.7]]
		self.T = self.calCovMatrix()
		self.NOISE_FLOOR = self.NF + intl.quad(ci.calC2,-2,2,args=(self.T[0][0]))[0]
		self.OUTPUT_POWER = self.PR_DBM + intl.dblquad(ci.calC1,-2,2,lambda t22:-2,lambda t22:2,args=(self.T[0][1],self.T[1][1]))[0]
		self.DATA_RATE = argv['DATA_RATE']

		self.P_TEMP = intl.dblquad(ci.calC,-2*self.SHADOWING_STANDARD_DEVIATION,2*self.SHADOWING_STANDARD_DEVIATION,lambda dist:self.DISMIN,lambda dist:self.DISMAX,args=(self.NOISE_FLOOR,self.DATA_RATE,self.BW,self.PATH_LOSS_EXPONENT,self.LAMBDA,self.OUTPUT_POWER,self.NOISE,self.D0,self.PREAMBLE_LENGTH,self.FRAME_LENGTH,self.S,self.PR_DBM))[0]
